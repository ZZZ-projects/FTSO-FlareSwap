...
// ─── Config – make sure these live in your .env ───────────────────────────────
...
// ─── Helpers – pick the correct classes & utils for your ethers version ──────
...
async function main() {
  ...
  for (const sym of wanted) {
    ...
    if (i == null) {
      ...
      const timestamp = rawTss[i].toNumber();
      console.log(
        `${sym}: ${price} FPREC  @ ${new Date(timestamp * 1000).toISOString()}`
      );
    }
  }
}
